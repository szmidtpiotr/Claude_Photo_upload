#!/usr/bin/env bash
# Screenshot Upload Server — installer
# Usage: bash install.sh
set -e

INSTALL_DIR="$HOME/.local/share/screenshot-upload"
SYSTEMD_DIR="$HOME/.config/systemd/user"
PORT=37701

echo "Installing screenshot upload server on port $PORT..."
...
cat > "$SYSTEMD_DIR/screenshot-upload.service" << 'EOF'
[Unit]
Description=Screenshot upload server (port 37701)
After=default.target

[Service]
Type=simple
ExecStart=/usr/bin/python3 %h/.local/share/screenshot-upload/server.py
Restart=on-failure
RestartSec=5
...
cat > "$SYSTEMD_DIR/screenshot-cleanup.service" << 'EOF'
[Unit]
Description=Delete screenshots older than 2 days

[Service]
Type=oneshot
ExecStart=/usr/bin/find %h/screenshots -type f -mtime +2 -delete
EOF

cat > "$SYSTEMD_DIR/screenshot-cleanup.timer" << 'EOF'
[Unit]
Description=Daily screenshot cleanup
...
systemctl --user enable --now screenshot-upload.service
systemctl --user enable --now screenshot-cleanup.timer

echo ""
echo "Done! Server running at http://$(hostname -I | awk '{print $1}'):$PORT"
echo "Screenshots saved to: $HOME/screenshots/"
